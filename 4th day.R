...
library(caret)
bg_indx <- createDataPartition(credit_bg$default.payment.next.month, p=.8, list=F)
credit_bg1 <- credit_bg[bg_indx, ] # 훈련 데이터
credit_bg2 <- credit_bg[-bg_indx, ] # 시험 데이터
...
#### <C5.0>
library(C50)
c50_idx <- createDataPartition(credit_bg$default.payment.next.month, p=.8, list=F)

credit_c50_train <- credit_bg[c50_idx, ]
credit_c50_test <- credit_bg[-c50_idx,]
...
rf_temp_sub <- randomForest(Y18~temp1+pres1+wins1+wins2, data=temperature_train[-1], ntree=700)
#importance(rf_temp_sub)
