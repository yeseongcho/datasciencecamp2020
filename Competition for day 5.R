...
setdiff(tr$sido, te$sido)
setdiff(te$sido, tr$sido) 
union(tr$sido, te$sido)
...
summary(tr$employee1)  
boxplot(tr$employee1) --## 이상치 파악에 용이..!!

# 전주지역 부모의 수입을 파악 -- 이상치
# 2000, 3000, 2500, 10000000000, 1000000
  # 이 경우에는 level들을 다 통일해줘야 한다. 모든 경우를 다 포함하는 합집합의 level를 공통 레벨로 삼게 한다.
Sido <- union(tr$sido, te$sido)
